...
/* GD modified */
#define AZ_IOT_CHARGE_CONTROLLER_CLIENT_AGENT_MODEL_ID       "dtmi:GigaDevice:ChargeSensorController;1"
/* GD modified end */
...
/* Telemetry buffers */
static uint8_t ucScratchBuffer[ 700 ]; //700
static uint8_t ucPropertyBuffer[ 80 ];


/* Command buffers */
static uint8_t ucCommandResponsePayloadBuffer[ 256 ]; //128

/* Reported Properties buffers */
static uint8_t ucReportedPropertiesUpdate[ 1500 ]; //64
...
#define sampleaduPNP_COMPONENTS_LIST_LENGTH    2
static AzureIoTHubClientComponent_t pnp_components[ sampleaduPNP_COMPONENTS_LIST_LENGTH ] =
{
    ...
    azureiothubCREATE_COMPONENT( AZ_IOT_CHARGE_SENSOR_PROPERTIES_COMPONENT_NAME )
};
#else
static AzureIoTHubClientComponent_t pnp_components[ sampleaduPNP_COMPONENTS_LIST_LENGTH ] =
{
...
static void prvAzureDemoTask( void * pvParameters )
{
    ...
    for( ; ; )
    {
        if( xAzureSample_IsConnectedToInternet() )
        {
            ...
            xHubOptions.pucModelID = ( const uint8_t * ) AZ_IOT_CHARGE_CONTROLLER_CLIENT_AGENT_MODEL_ID;
            xHubOptions.ulModelIDLength = sizeof(AZ_IOT_CHARGE_CONTROLLER_CLIENT_AGENT_MODEL_ID) - 1;//AzureIoTADUModelIDLength;
            ...
            for( ; xAzureSample_IsConnectedToInternet(); )
            {
                ...
                ulReportedPropertiesUpdateLength = ulCreateReportedPropertiesUpdate( ucReportedPropertiesUpdate, sizeof( ucReportedPropertiesUpdate ) - 1);
...
void vStartDemoTask( void )
{
    ...
    xTaskCreate( prvAzureDemoTask,         /* Function that implements the task. */
                 ...
                 OS_TASK_PRIORITY( democonfigDEMO_TASKPRIO ),         /* Task priority, must be between 0 and configMAX_PRIORITIES - 1. */
                 NULL );                   /* Used to pass out a handle to the created task - not used in this case. */
}
/*-----------------------------------------------------------*/

#else /* CONFIG_AZURE_F527_DEMO_SUPPORT */

#include "sample_azure_iot_f527_api.c"
#include "sample_azure_iot_f527.c"
