...
// GD
#define sampleazureiotCOMMAND_Reboot_REPORT               "reboot"
#define sampleazureiotCOMMANDRebootResponse               "response"
...
#define sampleazureiotDEFAULT_CHARGE_STATUS               53.0 // 53.0%
#define sampleazureiotDEFAULT_INDICATELED_VALUE           0 // 0: OFF, 1: ON
#define sampleazureiotDOUBLE_DECIMAL_PLACE_DIGITS         2

static double xDeviceCurrentChargeStatus = sampleazureiotDEFAULT_CHARGE_STATUS;
static bool xDeviceCurrentIndicateLedValue = sampleazureiotDEFAULT_INDICATELED_VALUE;
...
#define sampleazureiotPROPERTY_INDICATE_LED_TEXT          "indicateLED"

/**
 * @brief Telemetry values
 */
#define sampleazureiotTELEMETRY_NAME                      "chargestatus"

/**
 *@brief The Telemetry message published in this example.
 */
#define sampleazureiotTELEMETRY_MESSAGE                   "{\"" sampleazureiotTELEMETRY_NAME "\":%0.2f}"
...
/*-----------------------------------------------------------*/
static void prvSystemRebootTimercb( void *ptmr, void *p_arg )
{
    LogInfo( ( "System is going to reboot\r\n" ) );
    SysTimer_SoftwareReset();
}

/*-----------------------------------------------------------*/
static AzureIoTResult_t prvInvokeRebootCommand( AzureIoTJSONReader_t * pxReader,
                                                AzureIoTJSONWriter_t * pxWriter )
{
    ...
    else if( ( xResult = AzureIoTJSONReader_GetTokenInt32( pxReader,
                                                            &ulDelay ) )
             != eAzureIoTSuccess )
    {
        LogError( ( "Error getting token integer: result 0x%08x", xResult ) );
    }

    sys_timer_init(&reboot_tmr, (const uint8_t *)("reboot_timer"), ulDelay * 1000, 0, prvSystemRebootTimercb, NULL);
    sys_timer_start(&reboot_tmr, 0);

    if( ( xResult = AzureIoTJSONWriter_AppendBeginObject( pxWriter ) )
             != eAzureIoTSuccess )
    {
        LogError( ( "Error appending begin object: result 0x%08x", xResult ) );
    }
    else if( ( xResult = AzureIoTJSONWriter_AppendPropertyWithInt32Value( pxWriter, ( const uint8_t * ) sampleazureiotCOMMANDRebootResponse,
                                                                           sizeof( sampleazureiotCOMMANDRebootResponse ) - 1,
                                                                           response_code) )
             != eAzureIoTSuccess )
    {
        LogError( ( "Error appending IndicateLed value: result 0x%08x", xResult ) );
    }
...
static void prvUpdateLocalIndicateLed( bool xUpdatedLedValue )
{
    LogInfo(("set indicate led to mode: %d\r\n", xUpdatedLedValue));
    xDeviceCurrentIndicateLedValue = xUpdatedLedValue;
}
/*-----------------------------------------------------------*/

/**
 * @brief Generate an update for the device's indicateLed property online,
 *        acknowledging the update from the IoT Hub.
 */
static uint32_t prvGenerateAckForIncomingLed( bool xUpdatedLedValue,
                                                      uint32_t ulVersion,
                                                      ...
                                                      uint32_t ulResponseBufferSize )
{
    ...
    /* Building the acknowledgement payload for the indicateLed property to signal we successfully received and accept it. */
    ...
    xResult = AzureIoTHubClientProperties_BuilderBeginComponent( &xAzureIoTHubClient, &xWriter, ( const uint8_t * ) AZ_IOT_CHARGE_SENSOR_PROPERTIES_COMPONENT_NAME,
                                                                      strlen( AZ_IOT_CHARGE_SENSOR_PROPERTIES_COMPONENT_NAME ) );
        configASSERT( xResult == eAzureIoTSuccess );

    xResult = AzureIoTHubClientProperties_BuilderBeginResponseStatus( &xAzureIoTHubClient,
                                                                      &xWriter,
                                                                      ( const uint8_t * ) sampleazureiotPROPERTY_INDICATE_LED_TEXT,
                                                                      sizeof( sampleazureiotPROPERTY_INDICATE_LED_TEXT ) - 1,
    ...
    xResult = AzureIoTJSONWriter_AppendBool( &xWriter, xUpdatedLedValue);
    configASSERT( xResult == eAzureIoTSuccess );
    ...
    {
        LogDebug( ( "Sending acknowledged writable property. Payload: %.*s", lBytesWritten, pucResponseBuffer ) );
        xResult = AzureIoTHubClient_SendPropertiesReported( &xAzureIoTHubClient, pucResponseBuffer, lBytesWritten, NULL );
    ...
    *pulWritablePropertyResponseBufferLength = 0;
                                                                                  ...
                                                                                  &pucComponentName, &ulComponentNameLength ) ) == eAzureIoTSuccess )
    {
        ...
        if( AzureIoTADUClient_IsADUComponent( &xAzureIoTADUClient, pucComponentName, ulComponentNameLength ) )
        {
            ...
            //LogInfo( ("Payload: %s", pxMessage->pvMessagePayload) );
            ...
            else if( AzureIoTJSONReader_TokenIsTextEqual( &xJsonReader,
                                                          ( const uint8_t * ) sampleazureiotPROPERTY_INDICATE_LED_TEXT,
                                                          sizeof( sampleazureiotPROPERTY_INDICATE_LED_TEXT ) - 1 ) )
        {
            ...
            /* Get Indicate Led value */
            xAzIoTResult = AzureIoTJSONReader_GetTokenBool( &xJsonReader, &xUpdatedLedValue );

            if( xAzIoTResult != eAzureIoTSuccess )
            {
                LogError( ( "Error getting the property version: result 0x%08x", xAzIoTResult ) );
                break;
            }

            xAzIoTResult = AzureIoTJSONReader_NextToken( &xJsonReader );
            configASSERT( xAzIoTResult == eAzureIoTSuccess );
            prvUpdateLocalIndicateLed( xUpdatedLedValue );
            *pulWritablePropertyResponseBufferLength = prvGenerateAckForIncomingLed (
                                                            xUpdatedLedValue,
                                                            ulPropertyVersion,
                                                            pucWritablePropertyResponseBuffer,
                                                            ulWritablePropertyResponseBufferSize );
                                           ...
                                           uint32_t ulPropertiesDataSize )
{
    ...
    /* Building the acknowledgement payload for the indicateLed property. */
    ...
    xResult = AzureIoTJSONWriter_AppendPropertyWithBoolValue( &xWriter, ( const uint8_t * ) sampleazureiotPROPERTY_INDICATE_LED_TEXT,
                                                                       sizeof( sampleazureiotPROPERTY_INDICATE_LED_TEXT ) - 1,
                                                                       xDeviceCurrentIndicateLedValue
                                                                       );
    configASSERT( xResult == eAzureIoTSuccess );
                          ...
                          uint32_t ulCommandResponsePayloadBufferSize )
{
    ...
    lCommandNameLength = sizeof( sampleazureiotCOMMAND_Reboot_REPORT ) - 1;

    if( ( lCommandNameLength == pxMessage->usCommandNameLength ) &&
        ( strncmp( sampleazureiotCOMMAND_Reboot_REPORT, ( const char * ) pxMessage->pucCommandName, lCommandNameLength ) == 0 ) )
    {
        /* Is for reboot cmd */
        ...
        /* Read from the writer the "Delay" value and use it to construct the response payload in the writer. */
        xResult = prvInvokeRebootCommand( &xReader, &xWriter );
        if( xResult == eAzureIoTSuccess )
        {
                            ...
                            uint32_t * ulTelemetryDataLength )
{
    int result = snprintf( ( char * ) pucTelemetryData, ulTelemetryDataSize,
                           sampleazureiotTELEMETRY_MESSAGE, xDeviceCurrentChargeStatus );
